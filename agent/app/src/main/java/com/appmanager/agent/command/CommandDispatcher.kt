        ...
        when (msg.type) {
            ...
            "command" -> {
                ...
                when (msg.action) {
                    ...
                    CommandAction.GET_MDM_STATUS -> MdmCommandHandler.getMdmStatus(msg, service)
                    CommandAction.GET_NTP_CONFIG -> MdmCommandHandler.getNtpConfig(msg, service)
                    CommandAction.SET_NTP_CONFIG -> MdmCommandHandler.setNtpConfig(msg, service)
                    CommandAction.SET_MDM_MODE   -> MdmCommandHandler.setMdmMode(msg, service)
                    // Device Owner 策略命令
                    CommandAction.MDM_LOCK_NOW              -> DpmCommandHandler.lockNow(msg, service)
                    CommandAction.MDM_WIPE_DEVICE           -> DpmCommandHandler.wipeDevice(msg, service)
                    CommandAction.MDM_REBOOT                -> DpmCommandHandler.reboot(msg, service)
                    CommandAction.MDM_SET_CAMERA            -> DpmCommandHandler.setCameraDisabled(msg, service)
                    CommandAction.MDM_SET_SCREEN_CAPTURE    -> DpmCommandHandler.setScreenCaptureDisabled(msg, service)
                    CommandAction.MDM_SET_PASSWORD_POLICY   -> DpmCommandHandler.setPasswordPolicy(msg, service)
                    CommandAction.MDM_SET_USER_RESTRICTION  -> DpmCommandHandler.setUserRestriction(msg, service)
                    CommandAction.MDM_SET_APP_HIDDEN        -> DpmCommandHandler.setAppHidden(msg, service)
                    CommandAction.MDM_SET_UNINSTALL_BLOCKED -> DpmCommandHandler.setUninstallBlocked(msg, service)
                    CommandAction.MDM_SET_PERMISSION_GRANT  -> DpmCommandHandler.setPermissionGrant(msg, service)
                    CommandAction.MDM_SET_TIME              -> DpmCommandHandler.setTime(msg, service)
                    CommandAction.MDM_SET_TIMEZONE          -> DpmCommandHandler.setTimeZone(msg, service)
                    CommandAction.MDM_SET_SECURE_SETTING    -> DpmCommandHandler.setSecureSetting(msg, service)
                    CommandAction.MDM_SET_KIOSK             -> DpmCommandHandler.setKioskMode(msg, service)
                    CommandAction.MDM_GET_POLICY_SNAPSHOT   -> DpmCommandHandler.getPolicySnapshot(msg, service)
                    CommandAction.MDM_REVOKE_DEVICE_OWNER   -> DpmCommandHandler.revokeDeviceOwner(msg, service)
                    CommandAction.MDM_CLEAR_ALL_POLICIES    -> {
                        DpmCommandHandler.clearAllPolicies(service)
                        CommandDispatcher.sendResult(service, msg.commandId, true, "所有策略已清除")
                    }
                    // Settings.Global/Secure（WRITE_SECURE_SETTINGS）
                    CommandAction.MDM_SET_ADB               -> SettingsCommandHandler.setAdb(msg, service)
                    CommandAction.MDM_SET_ANIMATION         -> SettingsCommandHandler.setAnimation(msg, service)
                    CommandAction.MDM_SET_LOCATION_MODE     -> SettingsCommandHandler.setLocationMode(msg, service)
                    CommandAction.MDM_ENABLE_ACCESSIBILITY  -> SettingsCommandHandler.enableAccessibility(msg, service)
                    CommandAction.MDM_SET_AUTO_TIME         -> SettingsCommandHandler.setAutoTime(msg, service)
                    CommandAction.MDM_SET_WIFI              -> SettingsCommandHandler.setWifi(msg, service)
                    CommandAction.MDM_SET_BLUETOOTH         -> SettingsCommandHandler.setBluetooth(msg, service)
                    CommandAction.MDM_SET_AIRPLANE          -> SettingsCommandHandler.setAirplaneMode(msg, service)
                    CommandAction.MDM_SET_STAY_ON           -> SettingsCommandHandler.setStayOn(msg, service)
                    CommandAction.MDM_SET_UNKNOWN_SOURCES   -> SettingsCommandHandler.setUnknownSources(msg, service)
                    CommandAction.MDM_SET_ZEN_MODE          -> SettingsCommandHandler.setZenMode(msg, service)
                    CommandAction.MDM_SET_GLOBAL_SETTING    -> SettingsCommandHandler.setGlobalSetting(msg, service)
                    CommandAction.MDM_GET_SYSTEM_SETTINGS   -> SettingsCommandHandler.getSystemSettings(msg, service)
