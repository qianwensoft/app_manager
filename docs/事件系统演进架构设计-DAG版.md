...
| _（第 5-6 步：窄 DAG 调度器 + xyflow 画布）_ | 自研精简 schema + 拓扑调度 + 独立设计器微应用 | **待细化** | 建议 1-4 步落地、攒够 2-4 个真实场景后开工 |
| _（第 7-8 步：跨设备 STOMP 中继 + yjs 协同）_ | 跨 form-app 传输 + CRDT 协同编辑 | **待细化** | 前置：评审 A2 载荷契约 |

> **阅读顺序**：先本文第一～三节（建立全局）→ 第九节对抗性评审（看清张力与已决项）→ 再按需读各步落地设计。**动手顺序**：第 1 → 2 → 3-4 步（各自独立可交付、不依赖 DAG），第 5 步起按结论择机。
