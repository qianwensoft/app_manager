...
| `事件引擎脱Formily落地设计.md` | **第 1 步**：`StateScope`/`PageState`，事件引擎脱 Formily | 已细化 | 公共前置（5-8 步都依赖）|
| `第2步-AppState落地设计.md` | **第 2 步**：AppState + `state_change` 源 + app 级常驻事件 + 跨页范式 | 已细化 | 依赖第 1 步；答总纲待决 A3 |
| `第3-4步-ToolRegistry与降级守卫落地设计.md` | **第 3-4 步**：9 动作收敛为工具注册表 + 超时/重试/错误策略 + 环路守卫 | 已细化 | 依赖第 1-2 步；修 emit 死循环 |
| _（第 5 步：窄 DAG 调度器）_ `第5步-窄DAG调度器落地设计.md` | 自研精简 schema + 拓扑 BFS 调度 + $node 传递 + 静态环检测 + NodeTrace | **已细化（待开工）** | 复用 1-4 步底座≥80%；交付后能跑 DAG 但需第 6 步才能配 |
| _（第 6 步：xyflow 画布）_ | 独立 React19 设计器微应用，绑自研 schema | **待细化** | 与第 5 步连续做才「能配且能跑」 |
| _（第 7-8 步：跨设备 STOMP 中继 + yjs 协同）_ | 跨 form-app 传输 + CRDT 协同编辑 | **待细化** | 前置：评审 A2 载荷契约 |

> **阅读顺序**：先本文第一～三节（建立全局）→ 第九节对抗性评审（看清张力与已决项）→ 再按需读各步落地设计。**动手顺序**：第 1 → 2 → 3-4 步**已实现并单测**；第 5 步设计就绪、待决定是否在画布前先交付纯运行时。
