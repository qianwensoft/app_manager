...
| `A2-跨设备事件载荷契约设计.md` | **A2**：跨 form-app / 跨设备事件的载荷契约、安全边界、动作白名单 | ✅ 已设计（待评审决策点） | 第 7-8 步硬前置 |
| _（第 7-8 步：跨设备 STOMP 中继 + yjs 协同）_ | 跨 form-app 传输 + CRDT 协同编辑 | **待细化** | 前置：A2 契约拍板 |

> **阅读顺序**：先本文第一～三节（建立全局）→ 第九节对抗性评审（看清张力与已决项）→ 再按需读各步落地设计。**实施进度**：第 1-5 步**已实现并验证**（56 单测 + 19 E2E + 已合并 main）；第 6 步待开，第 7-8 步待 A2 决策点拍板。
