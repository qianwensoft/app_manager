...
- Name: yjs-collaboration.spec.ts >> Yjs 协同编辑功能 >> 应该显示正确的连接状态
- Location: tests/yjs-collaboration.spec.ts:175:7
...
Locator: locator('div.rounded-lg').filter({ hasText: /已连接|人在线/ }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('div.rounded-lg').filter({ hasText: /已连接|人在线/ }).first()
